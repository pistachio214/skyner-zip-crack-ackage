...
public class CrackUtil {
    ...
    public static String crack(String source, String dest, int minLen, int maxLen, DictionaryService dictionary) {
        ...
        LogUtil.info("\n[阶段0] 加载历史密码记录...");
        // 计算文件的 MD5
        currentFileMD5 = FileHashUtil.calculateMD5(source);
        if (currentFileMD5 == null) {
            LogUtil.error("无法计算文件的 MD5，将不会记录失败的密码");
            failedPasswords = new HashSet<>();
            LogUtil.info("完成 - MD5 计算失败，跳过历史记录加载");
        } else {
            failedPasswords = FileHashUtil.getFailedPasswords(currentFileMD5);
            LogUtil.info("完成 - 已加载 " + failedPasswords.size() + " 个历史失败密码记录");
        }
        ...
        try {
            ...
            if (result != null) {
                LogUtil.clearProgress(); // 清除进度行
                FileHashUtil.clearFailedPasswords(currentFileMD5); // 清除失败记录
                LogUtil.info("");
                LogUtil.info("破解成功！密码是: " + result);
                LogUtil.info("文件已解压到: " + dest);
                return result;
            }
            FileHashUtil.flushFailedPasswords(currentFileMD5); // 阶段1结束时写入
            LogUtil.clearProgress(); // 清除进度行
            LogUtil.info("完成 - 尝试了 " + stageAttempts + " 个常用密码，跳过了 " + stageSkipped + " 个已知密码");
            ...
            FileHashUtil.flushFailedPasswords(currentFileMD5); // 阶段2结束时写入
            LogUtil.clearProgress(); // 清除进度行
            LogUtil.info("完成 - 尝试了 " + stageAttempts + " 个数字组合，跳过了 " + stageSkipped + " 个已知密码");
            ...
            FileHashUtil.flushFailedPasswords(currentFileMD5); // 阶段3结束时写入
            LogUtil.clearProgress(); // 清除进度行
            LogUtil.info("完成 - 尝试了 " + stageAttempts + " 个模式密码，跳过了 " + stageSkipped + " 个已知密码");
            ...
            if (result != null) {
                ...
                FileHashUtil.flushFailedPasswords(currentFileMD5); // 破解失败时写入
                LogUtil.clearProgress(); // 清除进度行
                LogUtil.info("完成 - 尝试了 " + stageAttempts + " 个暴力破解密码，跳过了 " + stageSkipped + " 个已知密码");
                LogUtil.info("");
                LogUtil.info("破解失败 - 总共尝试了 " + totalAttempts.get() + " 个密码，跳过了 " + skippedAttempts + " 个已知密码，耗时 " +
                        String.format("%.1f", (System.currentTimeMillis() - startTime) / 1000.0) + " 秒");
            }

        } catch (Exception e) {
            LogUtil.error("破解过程中发生错误: " + e.getMessage());
            e.printStackTrace();
        }
    ...
    private static String tryPasswordList(String source, String dest, String[] passwords) {
        for (String password : passwords) {
            ...
            // 记录失败的密码
            if (currentFileMD5 != null) {
                FileHashUtil.recordFailedPassword(currentFileMD5, password);
            }
    ...
    private static boolean tryPassword(String source, String dest, String password) {
        return CompressUtil.extract(source, dest, password);
    }
    ...
    private static void reportProgress(String lastTried, int totalTries) {
        ...
        LogUtil.progress(String.format("\r尝试: %-20s [总尝试: %d] [已跳过: %d] [速度: %.1f次/秒] [耗时: %.1f秒]",
    ...
    private static void reportProgress(String lastTried, int totalTries, int currentLen) {
        double timeElapsed = (System.currentTimeMillis() - startTime) / 1000.0;
        double speed = totalTries / timeElapsed;
        LogUtil.progress(String.format("\r尝试: %-20s [长度: %d] [总尝试: %d] [已跳过: %d] [速度: %.1f次/秒] [耗时: %.1f秒]",
